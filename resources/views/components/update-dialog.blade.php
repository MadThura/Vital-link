 <div x-show="updateDialog" x-cloak x-transition:enter="ease-out duration-300" x-transition:enter-start="opacity-0"
     ...
     <div @click.away="updateDialog = false" x-show="updateDialog" x-transition:enter="ease-out duration-300"
         ...
         <!-- Content -->
         <form @submit.prevent="confirmDonation" class="p-6 space-y-6">
    ...
    <div class="space-y-4">
        ...
        <div>
            ...
            <input id="units" name="units" x-model="units" type="number" min="1" max="10"
                class="w-full bg-gray-700 border border-gray-600 rounded-lg px-4 py-2.5 text-white 
                       focus:ring-1 focus:ring-cyan-500 focus:border-cyan-500 transition-all" required>
        </div>

        <!-- Notes -->
        <div>
            <label for="note" class="block text-sm font-medium text-cyan-300 mb-1">Notes</label>
            <textarea id="note" name="note" x-model="note" rows="3"
                class="w-full bg-gray-700 border border-gray-600 rounded-lg px-4 py-2.5 text-white 
    ...
    <div class="grid grid-cols-2 gap-3">
        <div class="bg-gray-700/40 p-3 rounded-lg text-center">
            ...
            <p class="text-cyan-300 font-semibold">12</p>
        </div>
        <div class="bg-gray-700/40 p-3 rounded-lg text-center">
            <p class="text-gray-400 text-xs">Eligible</p>
            <p class="text-emerald-300 font-semibold">Yes</p>
        </div>
    </div>
    ...
    <div class="bg-gray-700/30 p-4 rounded-lg border border-gray-600/30">
        ...
        <ul class="text-xs text-gray-300 space-y-1">
            <li class="flex items-center"><i class="fas fa-venus-mars text-gray-400 mr-2 w-4"></i> Female, 32 years</li>
            <li class="flex items-center"><i class="fas fa-weight text-gray-400 mr-2 w-4"></i> 58 kg</li>
            <li class="flex items-center"><i class="fas fa-heartbeat text-gray-400 mr-2 w-4"></i> Normal pulse</li>
        </ul>
    </div>
